...
class Emojigame extends Component {
  ...
  onChangeState = () => {
    this.setState(prevState => ({isGameStart: !prevState.isGameStart}))
    this.setState({time: 60, score: 0, showImage: imagesList[0]})
    this.componentDidMount()
  }

  onImageClicked = imageId => {
    const {showImage, isGameStart} = this.state
    const {id} = showImage
    ...
    if (id === imageId) {
      ...
      this.setState({
        showImage: imagesList[Math.floor(Math.random() * imagesList.length)],
      })
    } else {
      this.setState({isGameStart: !isGameStart})
  ...
  renderWinLoose = () => {
    ...
    return <WinLoose scores={score} pleaseAgainGame={this.onChangeState} />
  }

  filterItems = () => {
    const {isActiveTabId} = this.state
    const filterImage = imagesList.filter(
      eachImage => eachImage.category === isActiveTabId,
    )
    return filterImage
  }

  renderImagesList = () => {
    const {isActiveTabId, showImage} = this.state
    const {imageUrl} = showImage
    const filterListImage = this.filterItems()
        ...
        <ul className="tab-list-container">
          {tabsList.map(eachTab => (
            <TabItem
              ...
              key={eachTab.tabId}
              isActive={eachTab.tabId === isActiveTabId}
              onTabClick={this.onTabClicked}
            />
          ))}
        </ul>
        <ul className="image-list-container">
          {filterListImage.map(eachImage => (
            <ImageItem
              imageDetails={eachImage}
              key={eachImage.id}
              onClickTabImage={this.onImageClicked}
            />
          ))}
        </ul>
      </div>
  ...
  render() {
    const {isGameStart, time, score} = this.state
    return (
      <div className="app-container">
        ...
        />
        <div>
          {isGameStart ? this.renderImagesList() : this.renderWinLoose()}
        </div>
      </div>
    )
  }
